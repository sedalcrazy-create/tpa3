...
import type { SocialWork } from '../../types/center';
...
export default function SocialWorkListPage() {
  ...
  const { data: response, isLoading } = useQuery({
    queryKey: ['social-work', queryParams],
    queryFn: () => commissionApi.socialWork.list(queryParams),
    placeholderData: keepPreviousData,
  });

  const records: SocialWork[] = response?.data ?? [];
  ...
  // ---- Columns definition -------------------------------------------------
  const columns = useMemo<Column<SocialWork & Record<string, unknown>>[]>(
    () => [
      {
        key: 'id',
        title: 'شناسه',
  ...
  const renderActions = useCallback(
    (row: SocialWork & Record<string, unknown>) => (
      <>
        <Link
          to={`/commission/social-work/${row.id}`}
        ...
        <Link
          to={`/commission/social-work/${row.id}/edit`}
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-bold text-gray-800">مددکاری اجتماعی</h1>
        <Link to="/commission/social-work/create">
      ...
      <div className="flex items-center gap-3 flex-wrap">
        <SearchInput
          ...
          placeholder="جستجوی پرونده مددکاری..."
          className="w-72"
        />
      </div>

      {/* Data table */}
      <DataTable<SocialWork & Record<string, unknown>>
        columns={columns}
        data={records as (SocialWork & Record<string, unknown>)[]}
        loading={isLoading}
        keyField="id"
        sortBy={sortBy}
        sortOrder={sortOrder}
