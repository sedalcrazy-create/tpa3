...
export default function EmployeeEditPage() {
  ...
  const { data: employee, isLoading: employeeLoading, isError } = useQuery({
  ...
  const {
    ...
    defaultValues: {
      custom_employee_code_id: '', special_employee_type_id: '', parent_id: '',
      first_name: '', last_name: '', father_name: '', phone: '', mobile: '',
      personnel_code: '', bank_account_number: '', id_number: '', email: '',
      address: '', photo: null, relation_type_id: '', national_code: '',
      gender: '', guardianship_type_id: '', marriage_status_id: '',
      location_id: '', location_work_id: '', birth_date: '', employment_date: '',
      branch_id: '', priority: '', description: '', is_active: true, status: '',
    },
  });

  // Pre-fill form from nested resource format
  useEffect(() => {
    if (employee) {
      reset({
        custom_employee_code_id: employee.custom_employee_code?.id != null ? String(employee.custom_employee_code.id) : '',
        special_employee_type_id: employee.special_employee_type?.id != null ? String(employee.special_employee_type.id) : '',
        parent_id: employee.parent?.id != null ? String(employee.parent.id) : '',
        ...
        photo: null,
        relation_type_id: employee.relation_type?.id != null ? String(employee.relation_type.id) : '',
        national_code: employee.national_code || '',
        gender: employee.gender?.value || '',
        guardianship_type_id: employee.guardianship_type?.id != null ? String(employee.guardianship_type.id) : '',
        marriage_status_id: employee.marriage_status?.id != null ? String(employee.marriage_status.id) : '',
        location_id: employee.location?.id != null ? String(employee.location.id) : '',
        location_work_id: employee.location_work?.id != null ? String(employee.location_work.id) : '',
        birth_date: employee.birth_date || '',
        employment_date: employee.employment_date || '',
        branch_id: employee.branch_id != null ? String(employee.branch_id) : '',
        priority: employee.priority != null ? String(employee.priority) : '',
  ...
  const updateMutation = useMutation({
    mutationFn: (formData: FormData) => employeesApi.update(numericId, formData),
    onSuccess: () => { toast.success('پرسنل با موفقیت ویرایش شد'); navigate('/employees'); },
    onError: () => { toast.error('خطا در ویرایش پرسنل'); },
  });

  const onSubmit = (values: EmployeeFormValues) => {
    const formData = new FormData();
    Object.entries(values).forEach(([key, value]) => {
      if (key === 'photo') { if (value instanceof File) formData.append('photo', value); return; }
  ...
  if (isError || !employee) {
    return (
      <div className="text-center py-32">
        ...
        <Button variant="secondary" onClick={() => navigate('/employees')}>بازگشت به لیست</Button>
      </div>
    );
  }

  const inputClass = 'w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-primary/30 focus:border-primary';

  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-bold text-gray-800">ویرایش پرسنل</h1>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8">
          ...
          <FormField label="کد ملی" required error={errors.national_code?.message}>
          ...
          <FormField label="تاریخ استخدام" error={errors.employment_date?.message}>
            <Controller name="employment_date" control={control}
              render={({ field }) => <DatePicker value={field.value || ''} onChange={field.onChange} />} />
          </FormField>
          <FormField label="تصویر" error={errors.photo?.message as string | undefined}>
            <Controller name="photo" control={control}
              render={({ field }) => <FileUpload value={field.value} onChange={field.onChange} accept="image/*" previewUrl={employee.photo || undefined} />} />
          </FormField>
          ...
          <FormField label="فعال" error={errors.is_active?.message}>
            <label className="inline-flex items-center gap-2 pt-2 cursor-pointer">
              <input type="checkbox" {...register('is_active')} className="w-4 h-4 text-primary border-gray-300 rounded focus:ring-primary" />
        ...
        <div className="flex items-center gap-3 mt-8 pt-6 border-t border-gray-200">
          <Button type="submit" loading={updateMutation.isPending}>ذخیره</Button>
